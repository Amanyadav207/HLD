...
class VisitCounterService:
    ...
    def _get_shard(self, page_id: str):
        ...
        return self.redis_manager.get_connection(page_id), shard_key

    def increment_visit(self, page_id: str) -> int:
        """
    ...
    def get_visit_count(self, page_id: str) -> dict:
        ...
        try:
            shard, shard_key = self._get_shard(page_id)
            count = shard.get(page_id)
            count = int(count) if count is not None else 0  # Ensure count is an integer
